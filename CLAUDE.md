...
- Employment & Projects order: ongoing roles (`-present`/`current`) first, sorted by most-recent start (Mountain Futures pinned top); then dated roles strictly reverse-chronological (by end year, then start year). Generated by an ad-hoc sort, but keep this order when hand-editing.

## Brand Colors (Mountain Futures)

- Forest green: `#639033` (download button, h2 in screen)
- Sky blue/teal: `#91ccd9` (h2, h3, subtitle)
- Dark grey: `#4b4b4b` (h1)
- Date labels (`code`): `#858585` grey in **both** screen and print
- Header contact line (`#webaddress`): `#aaa` grey in **both** screen and print

## CSS Notes

- Logo (`mf_logo.png`) displayed via `#main::before` pseudo-element (screen CSS and print CSS)
- The **logo IS shown in print** — top-left, ~150×60px, via `#main::before` in `cv-print.css`. Do not re-add a `display:none` for `#main::before` in `@media print` (it was removed from `cv-screen.css`)
- Only the **download button** is hidden in print (`@media print` in `cv-screen.css`)
